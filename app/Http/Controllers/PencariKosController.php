...
class PencariKosController extends Controller
{
    ...
    public function list()
    {
        ...
        if ($count > 0) {
            ...
            $datas = $this->paginate($datas, 6);
            $datas->setpath('/pencari-kos/list');
    ...
    public function filter(Request $request)
    {
        ...
        $datas = $this->paginate($datas, 6);
        $datas->setpath('/pencari-kos/list/filter?_method='.$request->_method.'&_token='.$request->token.'&Gender_kamar='.$request->Gender_kamar);
        return view('Page.Page_Pencari_Kos.list_kos', [
            'datas' => $datas
        ]);
        // return dd($Id_kos);
    ...
    public function updateProfile(Request $request, User $user)
    {
        ...
        if($validatedData['no_telepon'][0] == '0'){
            $special = '62';
            for($i=1; $i<Str::length($validatedData['no_telepon']); $i++){
                $special = $special . $validatedData['no_telepon'][$i];
            }
            $validatedData['no_telepon'] = $special;
